...
public class EtxSpatMqttDepositor extends AbstractEtxMqttDepositor {
    ...
    public void spatDepositListener(String message) {
        ...
        try {
            ...
            boolean wantMb = multiBrokerPublishService.isTargetActive(MqttBrokerTarget.MB)
                    && etxProperties.isMqttDepositorEnabled(MqttBrokerTarget.MB, "spat")
                    && etxProperties.passesSpatIntersectionFilter(MqttBrokerTarget.MB, spatMsg);
            if (!wantEtx && !wantNmi && !wantAv && !wantMb) {
                return;
            }
            ...
            Map<MqttBrokerTarget, BrokerPublishPayload> payloads =
                    new EnumMap<>(MqttBrokerTarget.class);
            if (wantEtx) {
                payloads.put(MqttBrokerTarget.ETX, BrokerPublishPayload.builder()
                        .target(MqttBrokerTarget.ETX).topics(topicSet).payload(etxPayload).build());
            }
            if (wantNmi) {
                Set<String> nmiTopicSet = NmiMqttTopicBuilder.getSpatTopicList(spatMsg,
                        mapDataCollector, etxProperties.mqttTopicPrecision(MqttBrokerTarget.NMI),
                        messageType);
            ...
            if (wantAv) {
                Set<String> avTopicSet =
                        NmiMqttTopicBuilder.getSpatTopicList(spatMsg, mapDataCollector,
                                etxProperties.mqttTopicPrecision(MqttBrokerTarget.AV), messageType);
                payloads.put(MqttBrokerTarget.AV,
                        BrokerPublishPayload.builder().target(MqttBrokerTarget.AV)
                                .topics(avTopicSet).payload(rawMessageBytes).build());
            }
            if (wantMb) {
                Set<String> mbTopicSet =
                        NmiMqttTopicBuilder.getSpatTopicList(spatMsg, mapDataCollector,
                                etxProperties.mqttTopicPrecision(MqttBrokerTarget.MB), messageType);
                payloads.put(MqttBrokerTarget.MB,
                        BrokerPublishPayload.builder().target(MqttBrokerTarget.MB)
