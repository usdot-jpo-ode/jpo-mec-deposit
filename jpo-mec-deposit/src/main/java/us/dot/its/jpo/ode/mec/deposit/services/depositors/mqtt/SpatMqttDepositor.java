...
/**
 * Depositor for handling SPAT messages via multi-broker MQTT fanout.
 */
@Component
@Slf4j
@ConditionalOnAnyMqttBrokerMqttDepositor("spat")
public class SpatMqttDepositor extends AbstractMqttDepositor {
   ...
   */
  @Autowired
  public SpatMqttDepositor(MecDepositProperties mecDepositProperties, EtxProperties etxProperties,
  ...
  public void spatDepositListener(String message) {
    ...
    try {
      ...
      if (wantNmi) {
        ...
        payloads.put(MqttBrokerTarget.NMI, BrokerPublishPayload.builder()
            .target(MqttBrokerTarget.NMI).topics(nmiTopicSet).payload(rawMessageBytes).build());
      }
      if (wantAv) {
      ...
      if (wantMb) {
        Set<String> mbTopicSet = NmiMqttTopicBuilder.getSpatTopicList(spatMsg, mapDataCollector,
